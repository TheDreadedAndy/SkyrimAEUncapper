...
static bool SkyrimUncapper_Initialize(const SKSEInterface* skse)
{
    ...
    if (!g_branchTrampoline.Create(1024 * 64, imgBase)) [[unlikely]]
    {
        _ERROR("couldn't create branch trampoline. this is fatal. skipping remainder of init process.");
        return false;
    }
    if (!g_localTrampoline.Create(1024 * 64, gDllHandle)) [[unlikely]]
    {
        _ERROR("couldn't create codegen buffer. this is fatal. skipping remainder of init process.");
        return false;
    }
    std::string path;
    if (!GetDllDirWithSlash(path)) [[unlikely]]
        return false;
    path += "SkyrimUncapper.ini";
    if (!settings.ReadConfig(path)) [[unlikely]]
        return false;
    Hook_Skill_Commit();
...
{
    if (skse->isEditor) [[unlikely]]
    { // ianpatt: yup no more editor. obscript gone (mostly)
        return false;
    }
    g_pluginHandle = skse->GetPluginHandle();
    //gDllHandle = reinterpret_cast<void *>(GetModuleHandleA("SkyrimUncapper.dll"));
